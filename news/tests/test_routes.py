...
# Импортируем класс модели новостей и комментов.
...
class TestRoutes(TestCase):
    ...
    def test_pages_availability(self):
        ...
        urls = (
            ...
            # Проверяем тут же страницы регистрации пользователей, входа в
            # учётную запись и выхода из неё доступные анонимным пользователям
            ('users:login', None),
            ('users:logout', None),
            ('users:signup', None),
        )
        ...
        for name, args in urls:
            with self.subTest(name=name):
                ...
                if name == 'users:logout':
                    response = self.client.post(url)  # Используем POST
                else:
